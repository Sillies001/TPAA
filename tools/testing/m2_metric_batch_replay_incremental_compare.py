#!/usr/bin/env python3
"""Cross-platform comparison for incremental M2-MET-007 batch/replay evidence."""
...
def compare(
    ...
    *,
    ...
    checks = {
        ...
        "tasks_exact": windows.get("task_id") == linux.get("task_id") == "M2-MET-007",
        ...
        "frozen_predecessors_equal": (
            windows.get("frozen_predecessors") == linux.get("frozen_predecessors")
        ),
        "blocked_predecessors_equal": (
            windows.get("blocked_predecessors") == linux.get("blocked_predecessors")
        ),
    ...
    return {
        "schema": (
            "TPAA_M2_MET_007_BATCH_REPLAY_INCREMENTAL_"
