# 飞机机载数据评估系统软件开发实施基线 SDIB-1.0.1

**Software Development Implementation Baseline — SDIB-1.0.1**  
**上位系统基线：** TPAA V8.0  
**工程设计基线：** ED-2.0  
...
**适用开发范围：** M0 Engineering Bootstrap 与 M1 P1 Minimum Vertical Slice 为实施级详细基线；M2～M9 为后续 Epic/Entry-Gate 级路线基线。  
**发布日期：** 2026-09-22
**修订性质：** 实施组织补丁版；不改变 R3.3 / CB-1.4.0 / Canonical 业务权威，不改变 116 项指标、661 输入绑定、Stage 或 DB schema 1.6.0。

### 0.0.1 本次修订摘要

SDIB-1.0.1 专门闭合 M0/M1 的“如何启动和按依赖施工”问题：

1. 为 M0 新增 `M0-DEV-000 Formal Repository Bootstrap`，使“建立正式软件仓库”拥有明确 Task ID；
2. 在 M0 Backlog 后增加 **启动顺序 ↔ Task ID 映射** 与六个施工波次；
3. 为 M1 增加独立的 **Entry Gate、启动清单、施工波次、Task ID 映射和第一条最窄 E2E Critical Path**；
4. 将原 §39 的重复启动列表改为 M0/M1 快速索引，避免两处维护同一规范性顺序；
5. 明确 Task ID、ADR ID、Milestone Transition Gate 三者分别管理“交付物、技术决策、里程碑转换”，不得互相替代。

> **文档权威边界**：SDIB-1.0.1 是“如何把已冻结设计实现成软件”的实施权威，不是新的业务 schema、DTO、Metric、Stage 或 P 能力定义权威。凡字段、枚举、nullability、Metric 公式、Stage code、P/M/WS 含义与本文件描述发生冲突时，必须以 CB-1.4.0 指向的 machine-readable Canonical authority 为准，并通过受控变更修订 SDIB，而不是在代码中静默偏离。
...
SDIB-1.0.1 回答 ED-2.0 之后开发团队必须立即面对的问题：代码仓怎么建、模块如何依赖、Canonical 如何进入代码、数据库如何 bootstrap、Release/Replay 如何先于大规模指标实现、Windows/Linux CI 怎么从第一天运行、M0/M1 做到什么程度才算完成，以及 M2～M9 在什么条件下才允许进入。
...
4. 本 SDIB-1.0.1 的实施组织、代码结构、Gate 与任务基线；
...
- **M2～M5**：Epic/能力包级规划；M1 Exit 后再根据真实运行反馈细化任务级 backlog。
...
SDIB-1.0.1 采用**单一主仓（monorepo）+ 清晰包边界**作为 M0 默认实现。原因是当前 Core/DTO/Metric/Stage/GUI/Windows-Linux 合同耦合较强，早期拆成多仓会增加版本同步与 contract drift 风险。未来如需拆仓，必须通过 ADR 与 contract package/versioning 证明不会破坏原子基线。
...
以下任务 ID 是 SDIB-1.0.1 的初始实施 backlog，共 **48 个 M0 工作包**。可以在项目管理工具中拆成更小 Task，但不得改变其验收语义；拆分项仍必须回指原 ID。
...
- 把 M1 的“代表Metric pipeline”扩成通用 Metric Engine；
- 32项 foundation batch 必须按 Catalog exact delivery metadata 实现；
- SNS family 在实现时必须执行 `RADAR` applicability，不能因 M2 指标批次而把它误用于 IRST/EO；
- Basic Flight 的训练工作流与 Sensor foundation batch 可以由不同 WS 并行推进，但都必须在 M2 Gate 汇合。

### Entry

M1 Exit 全部通过；MetricContext/Release/API/GUI 不再存在架构级 blocker。
...
远期实施细节必须在对应前置 M 的真实运行数据和验证证据基础上重新细化，不允许 SDIB-1.0.1 预先替未来模型/算法做未经验证的实现承诺。
...
- M1 Exit 后应发布 **SDIB-1.1**，把 M2 从 Epic 级细化为任务级，并吸收真实代码/Golden/跨平台反馈；
...
SDIB-1.0.1 的最终实施哲学可以概括为：**先把“正确、可追溯、可重放、跨平台一致”做成工程事实，再扩大功能覆盖；先证明一条真实闭环，再批量扩展指标和训练类型。**
