# 飞机机载数据评估系统软件开发实施基线 SDIB-1.0

**Software Development Implementation Baseline — SDIB-1.0**  
**上位系统基线：** TPAA V8.0  
**工程设计基线：** ED-2.0  
...
> **文档权威边界**：SDIB-1.0 是“如何把已冻结设计实现成软件”的实施权威，不是新的业务 schema、DTO、Metric、Stage 或 P 能力定义权威。凡字段、枚举、nullability、Metric 公式、Stage code、P/M/WS 含义与本文件描述发生冲突时，必须以 CB-1.4.0 指向的 machine-readable Canonical authority 为准，并通过受控变更修订 SDIB，而不是在代码中静默偏离。
...
SDIB-1.0 回答 ED-2.0 之后开发团队必须立即面对的问题：代码仓怎么建、模块如何依赖、Canonical 如何进入代码、数据库如何 bootstrap、Release/Replay 如何先于大规模指标实现、Windows/Linux CI 怎么从第一天运行、M0/M1 做到什么程度才算完成，以及 M2～M9 在什么条件下才允许进入。
...
4. 本 SDIB-1.0 的实施组织、代码结构、Gate 与任务基线；
...
SDIB-1.0 采用**单一主仓（monorepo）+ 清晰包边界**作为 M0 默认实现。原因是当前 Core/DTO/Metric/Stage/GUI/Windows-Linux 合同耦合较强，早期拆成多仓会增加版本同步与 contract drift 风险。未来如需拆仓，必须通过 ADR 与 contract package/versioning 证明不会破坏原子基线。
...
以下任务 ID 是 SDIB-1.0 的初始实施 backlog。可以在项目管理工具中拆成更小 Task，但不得改变其验收语义；拆分项仍必须回指原 ID。
...
远期实施细节必须在对应前置 M 的真实运行数据和验证证据基础上重新细化，不允许 SDIB-1.0 预先替未来模型/算法做未经验证的实现承诺。
...
## 39. M0 启动清单（从现在开始的实际顺序）

1. 建立正式软件仓库；
2. 导入 R3.3/CB-1.4.0 baseline snapshot 并验证 hash；
3. 建立 `pyproject + dependency lock + developer command` 骨架；
4. 关闭 ADR-M0-001～003，冻结 runtime/dependency/static toolchain；
5. 实现 Canonical loader + codegen；
6. 建 1.6.0 DB bootstrap + Repository skeleton；
7. 建 Application/FastAPI/PySide6 shell + version handshake；
8. 同时接通 Windows/Linux CI；
9. 建 test/fixture harness；
10. 完成其余 M0 backlog 与 cold-start；
11. M0 Exit Review；
12. 只在 M0 通过后开始 M1 synthetic vertical slice。

这是 SDIB-1.0 推荐的唯一主施工顺序。各 WS 可以并行，但依赖关系不能倒置。
...
SDIB-1.0 的最终实施哲学可以概括为：**先把“正确、可追溯、可重放、跨平台一致”做成工程事实，再扩大功能覆盖；先证明一条真实闭环，再批量扩展指标和训练类型。**
